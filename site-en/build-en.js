/* Smile Group EN — assembles translated drafts (_drafts/<slug>.js) into hizmet/<slug>.html
 * Mirrors ../site/build.js's escaping/validation approach, adapted for the EN schema
...
for (const f of files) {
  ...
  try {
    ...
    obj = sandbox.window.ARTICLE_EN;
    if (!obj) throw new Error('window.ARTICLE_EN bulunamadı');
  } catch (e) {
...
<script src="../assets/render-en.js?v=1"></script>
</body>
</html>
`;
  ...
  if (trLinkKalinti > 0) hatalar.push(`${slug}: ${trLinkKalinti} adet TR-stili /hizmet/ linki kalmış (mış olmalıydı /en/hizmet/)`);
...
console.log(`\nyazılan sayfa: ${yazilan} / beklenen 59 · index DONE: ${mevcut.length}`);
if (hatalar.length) {
  console.log('\n!! SORUNLAR:');
