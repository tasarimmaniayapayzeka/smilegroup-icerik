/* Smile Group DE BLOG — assembles _drafts/blog-<slug>.js drafts into blog/<slug>.html + blog/index.html */
const fs = require('fs');
...
const EDITOR = 'Dr. [Vor- und Nachname]';
...
const DE_SLUGS = JSON.parse(fs.readFileSync(path.join(DRAFTS, 'de-slug-map.json'), 'utf8')).blog;
...
for (const f of files) {
  ...
  try {
    ...
    obj = sandbox.window.BLOG_DE;
    if (!obj) throw new Error('window.BLOG_DE bulunamadı');
  } catch (e) {
...
<script src="../assets/render-blog-de.js?v=1"></script>
</body>
</html>
`;
  ...
  if (enLinkKalinti > 0) hatalar.push(`${slug}: ${enLinkKalinti} adet /en/ linki kalmış`);
...
const cards = ORDER.filter(s => articles.has(s)).map(slug => {
  ...
  const de = DE_SLUGS[slug] || slug;
  const trLine = tr ? `<span class="btr">TR: ${esc(tr)}</span>` : '';
  return `  <a class="bcard" href="${slug}.html"><img class="bimg" src="../images/${de}-detail-880x500.jpg" alt="${esc(a.title)} — Artikelbild" loading="lazy" onerror="this.remove()"><span class="bt">${esc(a.title)}</span>${trLine}<span class="bd">${esc(a.metaDescription || '')}</span></a>`;
}).join('\n');
...
<meta name="description" content="Smile-Group-Blogartikel — SEO/GEO-fertige, rechtssichere Inhalte, deutsche Ausgabe.">
<link rel="stylesheet" href="../assets/style.css?v=3">
<style>
...
<div class="topband"><div class="in">
  ...
  <span class="tag">Blog · <b>9 Artikel</b></span>
</div></div>
<div class="bwrap">
  <h1>Blogartikel</h1>
  <p class="sub">Bloginhalte, die die Behandlungsseiten speisen — datenverankert und rechtssicher.
     Jeder Artikel bringt fertiges WordPress-HTML plus Article-/FAQPage-Schemas mit.</p>
  <div class="bgrid">
${cards}
  </div>
</div>
<div class="foot">
  <span>Smile Group · Blog — interner Gebrauch.</span>
  <span><a href="../index.html">← Behandlungsinhalte</a></span>
</div>
</body>
</html>
`;
