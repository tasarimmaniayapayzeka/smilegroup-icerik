/* SITE-DE SON DENETİM — İngilizce merkezle yapısal eşlik + hata taraması.
...
console.log('EN html: ' + enF.length + ' · DE html: ' + deF.length);

/* stil dosyası */
if (!fs.existsSync(path.join(DE, 'assets', 'style.css'))) H.push('KRITIK: site-de/assets/style.css YOK — tüm sayfalar stilsiz kalır');

/* 2-6) sayfa taraması */
const kromKalinti = /(EN edition|Corporate page|Import to WordPress|>Copy<|← All content|min read|View details|Read More|Read the story|Read the article|View All|Book an Appointment|weeks ago|Illustrative image|Representative image|internal use\.|lang="en")/;
const linkler = new Map(); // hedef -> [kaynaklar]
for (const f of deF) {
  const p = path.join(DE, f);
  const s = fs.readFileSync(p, 'utf8');
  const temiz = s.replace(/TR[-: ][^\n]*/g, '').replace(/https?:[^\s"']*/g, '');
  if (!s.includes('lang="de"')) H.push('DİL: lang="de" yok → ' + f);
...
const hizmetTaslak = fs.readdirSync(path.join(DE, '_drafts'))
  .filter(x => x.endsWith('.js') && !x.startsWith('blog-') && !/^(sitewide-check|add-tr-labels|copy-de-images)/.test(x));
let sssEs = 0;
for (const t of hizmetTaslak) {
  const d = draft(DE, t, 'ARTICLE_DE');
...
for (const f of deF.filter(x => x.startsWith('hizmet/') || (x.startsWith('blog/') && !x.endsWith('index.html')))) {
  ...
  if (s.includes('render-de.js') || s.includes('render-blog-de.js')) sema++;
  else H.push('RENDER MOTORU YOK: ' + f);
}
console.log('render motorlu sayfa: ' + sema);

/* 9) slug haritaları */
const harita = JSON.parse(fs.readFileSync(path.join(DE, '_drafts', 'de-slug-map.json'), 'utf8'));
...
for (const v of [...Object.values(harita.hizmet), ...Object.values(harita.blog)]) {
  if (cift.has(v)) H.push('HARİTA: Almanca slug ÇİFT → ' + v);
  cift.add(v);
  if (/[äöüßÄÖÜ ]/.test(v)) H.push('HARİTA: slugda umlaut/boşluk → ' + v);
}
...
console.log('site-de/images .jpg: ' + imgDe + ' + doctors/: ' + fs.readdirSync(path.join(DE, 'images', 'doctors')).length);
