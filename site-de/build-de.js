...
const DE_SLUGS = JSON.parse(fs.readFileSync(path.join(DRAFTS, 'de-slug-map.json'), 'utf8')).hizmet;

const files = fs.readdirSync(DRAFTS).filter(f => f.endsWith('.js') && !f.startsWith('blog-') && !/^(sitewide-check|add-tr-labels|copy-de-images)/.test(f));
...
for (const f of files) {
  ...
  try {
    ...
    obj = sandbox.window.ARTICLE_DE;
    if (!obj) throw new Error('window.ARTICLE_DE bulunamadı');
  } catch (e) {
...
for (const [slug, a] of articles) {
  const article = {
    ...
    breadcrumb: ['Startseite', a.category, a.title],
...
<script src="../assets/render-de.js?v=1"></script>
</body>
</html>
`;
  ...
  if (trLinkKalinti > 0) hatalar.push(`${slug}: ${trLinkKalinti} adet TR-stili /hizmet/ linki kalmış (olması gereken /de/hizmet/)`);
